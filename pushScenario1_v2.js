...
async function pushScenario1V2() {
    const dateOnly = "2026-05-15";
    const timeOnly = "00:30:00"; 
    const createdOn = `${dateOnly} ${timeOnly}`;
    const orderID = `FITGREEN-TEST-401`;
            ...
            "OrderInfo": {
                ...
                "Order": {
                    "details": {
                        ...
                        "order_type": "H",
                        "payment_type": "ONLINE",
                        "total": "483.00", // Corrected total: 460 + 23
                        "tax_total": "23.00",
                        "discount_total": "0.00",
                        "discount_type": "F",
                        "description": "Scenario 1 - NEW TAX WRAPPER",
    ...
    console.log(`🚀 Executing Scenario 1 V2 (New Tax Structure: ${orderID})...`);
    try {
        const response = await axios.post(SAVE_ORDER_URL, payload);
        console.log(`✅ Response:`, JSON.stringify(response.data));
        
        fs.writeFileSync('scenario1_v2_request.json', JSON.stringify(payload, null, 2));
        fs.writeFileSync('scenario1_v2_response.json', JSON.stringify(response.data, null, 2));
    } catch (error) {
