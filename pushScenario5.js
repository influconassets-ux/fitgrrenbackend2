...
async function pushScenario5() {
    const dateOnly = "2026-05-15";
    const timeOnly = "01:10:00"; 
    const createdOn = `${dateOnly} ${timeOnly}`;
    const orderID = `FITGREEN-TEST-405`;
            ...
            "OrderInfo": {
                ...
                "Order": {
                    "details": {
                        ...
                        "discount_total": "0.00",
                        "discount_type": "F",
                        "description": "Scenario 5 - Item with Addon + Variation + Tax",
    ...
    console.log(`🚀 Executing Scenario 5 (Full Test: ${orderID})...`);
    try {
        const response = await axios.post(SAVE_ORDER_URL, payload);
        console.log(`✅ Response:`, JSON.stringify(response.data));
        
        fs.writeFileSync('scenario5_request.json', JSON.stringify(payload, null, 2));
        fs.writeFileSync('scenario5_response.json', JSON.stringify(response.data, null, 2));
    } catch (error) {
